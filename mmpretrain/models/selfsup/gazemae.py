...
class GazeMAEViT(VisionTransformer):
    """Vision Transformer for GazeMAE pre-training.
    ...
    def __init__(self,
        ...
        self.pos_embed.requires_grad = False
        self.mask_ratio = mask_ratio
        self.clinical_ratio=clinical_ratio
    ...
    def binary_seg(self,x_attn, mode='otsu'):
        ...
        attention_gray = x_attn.cpu().numpy()
    ...
    def forward(
        ...
        if mask is None or False:
            ...
            # masking: length -> length * mask_ratio
            # x, mask, ids_restore = self.random_masking(x, self.mask_ratio)
            x, mask, ids_restore = self.gaze_masking(x,x_foreground, self.mask_ratio, self.clinical_ratio)
...
@MODELS.register_module()
class GazeMAE(BaseSelfSupervisor):
    """MAE.
    ...
    def loss(self, inputs: torch.Tensor, data_samples: List[DataSample],
        ...
        attention = torch.stack(
            [data_sample.gt_seg_map for data_sample in data_samples])
        latent, mask, ids_restore = self.backbone({"img":inputs,"attention":attention})
