...
class GazeMAEViT(VisionTransformer):
    ...
    def gaze_masking(self,x: torch.Tensor, mask_ratio: float = 0.75, clinical_ratio=0.5, preload=False):
        ...
        x_masked = torch.gather(
            x, dim=1, index=ids_visible.unsqueeze(-1).repeat(1,1,768))
    ...
    def forward(
        ...
        if mask is None or False:
            ...
            # masking: length -> length * mask_ratio
            # x, mask, ids_restore = self.random_masking(x, self.mask_ratio)
            x, mask, ids_restore = self.gaze_masking(x, self.mask_ratio, self.clinical_ratio, preload=False)
